...
pub enum DecodeError {
    ...
    UnknownType,
    OverFlow(Vec<u8>),
    TooShort(usize, usize), // (expected, actual)
    Invalid(String),
}
...
impl Element {
    ...
    /// and return an owned Banana object
    /// TODO return remainder, maybe consume incoming bytes, then to get 0-copy
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, DecodeError> {
        let (length_bytes, elt_type) = Self::length_type(bytes).unwrap();
        match elt_type {
            0x81 => Ok(Element::Integer(Self::dec_posint(length_bytes)? as i32)),
            0x83 => Ok(Element::Integer(Self::dec_negint(length_bytes)? as i32)),
            0x82 => Ok(Element::String(Self::dec_string(length_bytes, bytes)?)),
            0x84 => Ok(Element::Float(Self::dec_float(length_bytes, bytes)?)),
